...
.MEASURE TRAN fall_en_dissip INTEGRAL P(x1.xdut) FROM='t_z_f_i' TO='t_z_f_e'
.MEASURE TRAN rise_en_dissip INTEGRAL P(x1.xdut) FROM='t_z_r_i' TO='t_z_r_e'

.MEASURE TRAN input_cap_r INTEGRAL 'Isub(x1.xdut.b)/vdd' FROM=1e-9 TO=1.01e-9
.MEASURE TRAN input_cap_f INTEGRAL '-Isub(x1.xdut.b)/vdd' FROM=2.01e-9 TO=2.02e-9
.MEASURE TRAN input_cap PARAM='(input_cap_r+input_cap_f)/2'

.MEASURE TRAN output_cap_f INTEGRAL 'Isub(x1.xdut.z)/(0.98*vdd)' FROM='t_z_f_i' TO='t_z_f_e' 
.MEASURE TRAN output_cap_r INTEGRAL '-Isub(x1.xdut.z)/(0.98*vdd)' FROM='t_z_r_i' TO='t_z_r_e'
